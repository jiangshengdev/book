...
智能指针 `Rc<T>` 也不实现 `Sync`，原因与它不实现 `Send` 相同。`RefCell<T>` 类型（我们在第 15 章中讨论过）和相关的 `Cell<T>` 类型系列不实现 `Sync`。`RefCell<T>` 在运行时进行的借用检查实现不是线程安全的。智能指针 `Mutex<T>` 实现了 `Sync`，可以用于与多个线程共享访问，正如你在["对 `Mutex<T>` 的共享访问"][shared-access]<!-- ignore -->中看到的那样。
